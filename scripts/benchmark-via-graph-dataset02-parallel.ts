...
if (HELP) {
  console.log(`
Usage: bun run scripts/benchmark-via-graph-dataset02-parallel.ts [options]
...
Examples:
  bun run scripts/benchmark-via-graph-dataset02-parallel.ts --limit=100
  bun run scripts/benchmark-via-graph-dataset02-parallel.ts --concurrency=4 --quick
  bun run scripts/benchmark-via-graph-dataset02-parallel.ts --limit=200 --concurrency=8
`)
  process.exit(0)
}
...
function runSampleInWorker(
  ...
  return new Promise((resolve, reject) => {
    const workerPath = path.join(
      __dirname,
      "benchmark-via-graph-dataset02-worker.ts",
    )
